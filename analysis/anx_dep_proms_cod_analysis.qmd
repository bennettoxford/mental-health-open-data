...
df_fig_proms_anx_dep_usage <- proms_anx_dep_usage |>
  ...
  mutate(
    semantic_tag_group = case_when(
      semantic_tag == "observable entity" ~ "observable entity",
      TRUE ~ "other"
    ),
...
fig_proms_anx_dep <- df_fig_proms_anx_dep_usage |> 
    ggplot(
      aes(
        y = usage,
        x = end_date,
    ...
    facet_wrap(~semantic_tag_group, ncol = 1)

ggsave(
  filename = here("output", "figures", "cod_analysis", "proms", "fig_proms_anx_dep.png"),
  fig_proms_anx_dep,
  height = 6,
  width = 8
)
...
fig_proms_anx_dep_title <- df_fig_proms_anx_dep_usage |> 
  filter(semantic_tag_group == "Observable entity") |> 
    ggplot(
      aes(
        y = usage,
        x = end_date,
    ...
    geom_point(size = 2) +
    labs(
      title = "Trends of recorded PROMs scores for depression and anxiety in English primary care",
      x = "End date of yearly time interval",
      y = "Recorded events",
      colour = NULL,
      shape = NULL
...
ggsave(
  filename = here("output", "figures", "cod_analysis", "proms", "fig_proms_anx_dep_title.png"),
