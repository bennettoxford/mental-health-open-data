...
depr_anx_psy_tx_codelists <- get_codelist(
...
#Filter snomed codes in the codelists 
depr_anx_psy_tx_codes <- snomed_usage %>%
  filter(snomed_code %in% depr_anx_psy_tx_codelists$code)


# Generate summary of the codelists and usage 
depr_anx_psy_tx_code_counts <- depr_anx_psy_tx_codes |> 
  get_semantic_tag() |> 
...
gtsave(
  tab_all_psy_tx,
  here("output", "tables", "tab_all_psy_tx_sem_tag_usage.png")
  )
...
nhstt_code_usage <- depr_anx_psy_tx_codes %>%
...
gtsave(
  ...
  here("output", "tables", "tab_nhstt_sem_tag_usage.png")
  )
...
ggsave(
  filename = here("output", "figures", "fig_psy_tx_nhstt_trends.png"),
...
# Filter for codes with NHS TT search terms
cbt_ct_code_usage <- depr_anx_psy_tx_codes |>
...
ggsave(
  filename = here("output", "figures", "fig_psy_tx_cbt_ct_trends.png"),
...
# Filter for codes with NHS TT search terms
psych_code_usage <- depr_anx_psy_tx_codes |>
...
gtsave(
  ...
  here("output", "tables", "tab_psych_sem_tag_usage.png")
  )
...
ggsave(
  filename = here("output", "figures", "fig_psy_tx_psych_trends.png"),
