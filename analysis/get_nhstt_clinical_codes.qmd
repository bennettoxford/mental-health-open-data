...
tab_nhstt_snomed <- nhstt_snomed_table |> 
  group_by(semantic_tag) |>
   ...
   tab_header(
    title = md("**Summary of SNOMED CT codes in NHS Talking Therapies metadata**")
   ) |> 
   tab_source_note(
    source_note = md(
      "Data source: NHS Talking Therapies Annual Reports metadata (technical construction field)"
    )
   ) |> 
    tab_footnote(
      footnote = "One SNOMED CT code appears with multiple descriptions due to difference in American and British spelling.",
...
gtsave(
 tab_nhstt_snomed,
 here("output", "nhstt", "clinical_codes", "tab_nhstt_snomed_usage.png")
 )
...
tab_nhstt_icd <- nhstt_icd_table |> 
  gt() |>
    ...
    tab_style(
      ...
      tab_header(
        title = md("**Summary of ICD-10 codes in NHS Talking Therapies metadata**")
      ) |> 
      tab_source_note(
    source_note = md(
      "Data source: NHS Talking Therapies Annual Reports metadata (technical construction field)"
    ...
    tab_footnote(
      footnote = "Some ICD-10 codes may not be captured due to insufficient code specificity.",
      locations = cells_column_labels(columns = icd10_code)
    ) |>
      fmt_number(total_usage, decimals = 0) |>
      fmt_percent(ratio_usage, decimals = 2)
...
gtsave(
  tab_nhstt_icd,
  here("output", "nhstt", "clinical_codes", "tab_nhstt_icd_usage.png")
  )

tab_nhstt_icd
```

### Missing ICD 10 codes from OpenCodeCounts
```{r}
#list of missing ICD 10 codes
missing_icd_codes <- list(icd_codelist[!(icd_codelist %in% nhstt_icd_table$icd10_code)])
...
The ICD-10 codes without code usage are F42 and K58. 