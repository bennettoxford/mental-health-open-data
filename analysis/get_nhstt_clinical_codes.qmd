...
1.  Clinical code extraction :
    Clinical codes were extracted from NHS TT annual metadata using the `technical_construction` field. Custom string-based extraction functions (`str_extract_snomed()`, `str_extract_icd()`) were applied to identify embedded clinical codes for SNOMED CT and ICD-10 respectively.

2. Code list preparation
    Extracted codes were processed separately for each coding system. The lists were flattened, deduplicated and missing values removed to produce unique sets of SNOMED CT and ICD-10 codes represented in the NHS TT metadata.

3. Description and code usage mapping
    The `opencodecounts` package was used to obtain standardised code descriptions and usage counts. 
