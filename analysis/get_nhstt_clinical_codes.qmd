...
## Background
NHS Talking Therapies (NHS TT) metadata includes embedded clinical codes within the descriptive text field used to define measures. These codes capture clinical concepts such as therapy categories and conditions. 
Extracting these codes allows for clearer documentation of clinical concepts used within NHS TT measures and enables linkage to external code usage with OpenCodeCounts.
...
n_snomed_tab <- length(nhstt_snomed_table)
...
n_icd_tab <- length(nhstt_icd_table)
