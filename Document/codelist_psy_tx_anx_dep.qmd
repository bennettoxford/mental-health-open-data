...
#### Psychological therapies code usage
...
codelist_summary <- snomed_usage %>%
 ...
 knitr::kable(codelist_summary, caption = "Summary of depression and anxiety SNOMED CT codes")
 n_dep_anx_codes <- nrow(codelist_summary)
 n_dep_anx_codes
```
...
#| echo: false 
cod_summary_grouped <- codelist_summary %>%
  mutate(description = ifelse(usage < 20000, "Others", description)) %>%
  ...
  mutate(
    percent_usage = round(100 * usage / sum(usage), 1)
  ) %>%
  arrange(desc(usage)) %>%
  mutate(description = factor(description, levels = rev(description)))  # Fix order for horizontal bars

# Horizontal bar chart
ggplot(cod_summary_grouped, aes(x = description, y = usage)) +
  geom_bar(stat = "identity", fill = "steelblue") +
  coord_flip() +
  labs(
    title = "Distribution of Code Usage",
    x = "Therapies",
    y = "Usage"
  ) +
